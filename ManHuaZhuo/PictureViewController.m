...
-(void)tapShowBar:(UITapGestureRecognizer *)tapGesture
{
    ...
    if(locationPoint.x<turnPageWidth)
    {
        ...
        {
            [CommonHelper showCustomHud:[UIImage imageNamed:@"tips_failed"] title:@"已经是第一页了" targetView:self.view];
        }
        else {
    ...
    else if(locationPoint.x>picScrollView.frame.size.width-turnPageWidth)
    {
        ...
        {
            [CommonHelper showCustomHud:[UIImage imageNamed:@"tips_failed"] title:@"已经是最后一页了" targetView:self.view];
        }
        else {
            [self updateToPageIndex:currentPage+1];
...
-(void)loadView
{
    ...
    [btn setBackgroundImage:[UIImage imageNamed:@"nav_back_pressed"] forState:UIControlStateHighlighted];
...
-(void)orationAction:(id)sender
{
    ...
    switch (item.tag) {
        case kRotateBarTag://是否支持旋转
        {
            ...
            NSString *imgName=@"autorotate_on";
            if (shouldRotate) {
                imgName=@"autorotate_off";
            }
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    appDelegate.navBarBgImg=[UIImage imageNamed:@"nav_bg_ios4"];
}
